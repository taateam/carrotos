...
void ustrconcat(UNICODE_STRING *dest, UNICODE_STRING *src1, UNICODE_STRING *src2)
{
    ...
    memcpy((uint8_t *)dest->Buffer, (uint8_t *)src1->Buffer, src1->Length);
    memcpy((uint8_t *)dest->Buffer + (src1->Length / sizeof(uint16_t)), (uint8_t *)src2->Buffer, src2->Length);
...
void setup_base_name_full_path(char *path, struct loaded_dll *loaded_dll)
{
    ...
    memcpy(loaded_dll->full_name, (uint8_t *)path_u.buffer, path_u.len + 2);
    ustr256 name;
    extract_file_name_from_path_utf16(path, &name, 32);
    memcpy(loaded_dll->name, (uint8_t *)name.buffer, name.len + 2);
...
{
    struct loaded_dll *main = &loaded_dll[dll_id];
    main->DllBase = (void *)image_base_addr;
